...
class GuestLog(object):
    ...
    instance_cache = {}
    cached_swift_client = None

    @classmethod
    def swift_client(cls, context):
        if not cls.cached_swift_client:
            cls.cached_swift_client = create_swift_client(context)
        return cls.cached_swift_client

    @classmethod
    def instance(cls, context, log):
        if log not in cls.instance_cache:
            cls.instance_cache[log] = cls(context, log)
        return cls.instance_cache[log]

    @classmethod
    def list(cls, context, app, datastore_logs):
        return [cls.instance(context, l).show(app)
                for l in datastore_logs.keys()]
    ...
    def __init__(self, context, log):
        ...
        self.swift_client = GuestLog.swift_client(context)

    def show(self, app):
        dsl_info = self._datastore_logs()[self.log]
    ...
    def publish_log(self, app, disable):
        dsl_info = self._datastore_logs()[self.log]
        log_type, log_user, log_filename = dsl_info
        self._enable_log(app, self.log, log_filename, disable)
        if self.logging_enabled:
            log_dir = os.path.dirname(log_filename)
            utils.execute_with_timeout("sudo", "chmod", "a+rx", log_dir)
            utils.execute_with_timeout("sudo", "chmod", "a+r", log_filename)
    ...
    def _set_container_details(self):
        ...
        log_type, log_user, log_filename = dsl_info
        container_header = {
            'X-Container-Meta-Log-Name': self.log,
